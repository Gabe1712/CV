...
if __name__ == "__main__":
    ...
    learning_rate = 0.01
    batch_size = 128
    l2_reg_lambda = 0.0 #change the values of lambda here
    ...
    for i in range(10):
        ...
        #plt.savefig("task4b_softmax_weight_" + str(i)+"_.png" )
        plt.show()
    plt.close()
    ...
    for Lambda in l2_lambdas:
        model = SoftmaxModel(Lambda)
        ...
        List_val_history.append(val_history)
        List_norm.append(np.linalg.norm(model.w))
        
       
    for l, train_history, val_history in zip(l2_lambdas, List_train_history, List_val_history):
        plt.ylim([0.7, .93])
        plt.xlim([-100, 10000])
