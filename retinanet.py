...
class RetinaNet(nn.Module):
    def __init__(self, 
        ...
        self.classification_heads = nn.Sequential(
            ...
            nn.Conv2d(
                in_channels=num_channels,
                out_channels=num_boxes*self.num_classes,
                kernel_size=3,
                stride=1,
                padding=1
            )
...
#add
        ...
        for i in range(9):
            if(i%2==0):
                nn.init.constant_(self.regression_heads[i].bias.data,0)
                nn.init.constant_(self.classification_heads[i].bias.data,0)
        p=0.99
        nn.init.constant_(self.classification_heads[-1].bias.data[:self.num_boxes],np.log(p*(self.num_classes-1)/(1-p)))
    ...
    def regress_boxes(self, features):
        ...
        for idx, x in enumerate(features):
            bbox_delta = self.regression_heads(x).view(x.shape[0], 4, -1)                       #retirei idx
            bbox_conf = self.classification_heads(x).view(x.shape[0], self.num_classes, -1)        #retirei idx
